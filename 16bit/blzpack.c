...
void compress_file(const char *oldname, const char *packedname)
{
   ...
   while ((n_read = fread(data, 1, BUFSIZE, oldfile)) > 0)
   {
      ...
      put_uint32(header + 2*4, packedsize & 0x0000ffff);
      put_uint32(header + 3*4, is16_crc32_asm((unsigned char *)packed, packedsize));
      put_uint32(header + 4*4, n_read & 0x0000ffff);
      put_uint32(header + 5*4, is16_crc32_asm((unsigned char *)data, n_read));
...
void decompress_file(const char *packedname, const char *newname)
{
   ...
   while ((n_read = fread(header, 1, sizeof(header), packedfile)) == sizeof(header))
   {
      ...
      if (get_uint32(header + 3*4) != is16_crc32_asm((unsigned char *)packed, get_uint32(header + 2*4)))
      {
      ...
      if (get_uint32(header + 5*4) != is16_crc32_asm((unsigned char *)data, depackedsize))
      {
...
{
   printf("===============================================================================\n"
          "BriefLZ 16-bit example          Copyright (c) 2002-2004 by Joergen Ibsen / Jibz\n"
          "                                                            All Rights Reserved\n\n"
          "                                                  http://www.ibsensoftware.com/\n"
          "===============================================================================\n\n");
